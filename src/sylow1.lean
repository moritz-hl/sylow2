...
--α is monoid (defined in lean standard library)
--generalization of group action, because groups are monoids
class mul_action (α : Type u) (β : Type v) [monoid α] extends has_scalar α β :=
(one_smul : ∀ b : β, (1 : α) • b = b)
(mul_smul : ∀ (x y : α) (b : β), (x * y) • b = x • y • b)

section
--α monoid, β ist die Menge
...
--Monoid γ, Monoid α, f: γ -> α Mon.Hom., gibt Gruppenaktion auf γ x β 
...
--Durch Element α erzeugte Permutation β -> β
...
--(left_inv / right_inf sind Beweise)


variables {α} {β}

--Der daraus resultierende Gruppenhomomorphismus
